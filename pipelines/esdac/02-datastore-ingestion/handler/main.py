...
def handler(context, event):
    ...
    try:
        ...
        if 'crs' in  body['properties']:
            if body['properties']['crs']['type'] == 'PROJ4':
                proj4 = body['properties']['crs']['properties']['string']
                source_proj = Proj(proj4)
        ...
        context.logger.info("SOC feature '{0}' '{1}' successfully".format(
            id, 
...
class Helpers(object):
    ...
    def load_configs():
        ...
        FunctionConfig.target_collection = os.getenv('TARGET_COLLECTION','soc')
